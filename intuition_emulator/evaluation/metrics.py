...
def compare_persistence_vs_feedback_a(metrics: dict) -> dict:
    """Exp A: Does persistence (H=f(P)) outlast feedback-only (fixed H) during dormancy?
    Compares persistence_only vs feedback_only_h8 and feedback_only_h12.
    Outcomes: persistence_wins | feedback_wins | no_advantage
    """
    p_alive = metrics.get("baseline_alive_at_50", {}).get("persistence_only", False)
    p_time  = metrics.get("baseline_reactivation_times", {}).get("persistence_only")
    p_react = metrics.get("baseline_reactivated", {}).get("persistence_only", False)
    result  = {}

    for fb in ("feedback_only_h8", "feedback_only_h12"):
        fb_alive = metrics.get("baseline_alive_at_50", {}).get(fb, False)
        fb_time  = metrics.get("baseline_reactivation_times", {}).get(fb)
        fb_react = metrics.get("baseline_reactivated", {}).get(fb, False)

        if p_alive and not fb_alive:
            result[fb] = _cr("persistence_wins", "alive_at_50", 1.0)
        elif not p_alive and fb_alive:
            result[fb] = _cr("feedback_wins", "alive_at_50", 1.0)
        elif not p_alive and not fb_alive:
            result[fb] = _cr("no_advantage", "alive_at_50", None)
        elif p_react and not fb_react:
            result[fb] = _cr("persistence_wins", "reactivation", 1.0)
        elif not p_react and fb_react:
            result[fb] = _cr("feedback_wins", "reactivation", 1.0)
        elif p_react and fb_react and p_time is not None and fb_time is not None and fb_time > 0:
            margin = (fb_time - p_time) / fb_time
            if margin >= THRESHOLD:
                result[fb] = _cr("persistence_wins", "reactivation_time", margin)
            elif margin <= -THRESHOLD:
                result[fb] = _cr("feedback_wins", "reactivation_time", margin)
            else:
                result[fb] = _cr("no_advantage", "reactivation_time", margin)
        else:
            result[fb] = _cr("no_advantage", "reactivation_time", None)

    return result


def compare_persistence_vs_feedback_b(metrics: dict) -> dict:
    """Exp B: Does feedback alone (fixed H) recover better than persistence alone (no F)?
    Compares persistence_only vs feedback_only_h8/h12 on recovery_time.
    """
    p_rt   = metrics.get("baseline_recovery_times", {}).get("persistence_only")
    p_dead = metrics.get("baseline_dominant_dead", {}).get("persistence_only")
    p_fp   = metrics.get("baseline_false_projections", {}).get("persistence_only", 0)
    result = {}

    for fb in ("feedback_only_h8", "feedback_only_h12"):
        fb_rt   = metrics.get("baseline_recovery_times", {}).get(fb)
        fb_dead = metrics.get("baseline_dominant_dead", {}).get(fb)
        fb_fp   = metrics.get("baseline_false_projections", {}).get(fb, 0)

        if p_rt is not None and fb_rt is None:
            result[fb] = _cr("persistence_wins", "recovery_time", 1.0)
        elif p_rt is None and fb_rt is not None:
            result[fb] = _cr("feedback_wins", "recovery_time", 1.0)
        elif p_rt is None and fb_rt is None:
            result[fb] = _cr("no_advantage", "recovery_time", None)
        else:
            margin = (p_rt - fb_rt) / p_rt if p_rt > 0 else 0.0
            if margin >= THRESHOLD:
                result[fb] = _cr("feedback_wins", "recovery_time", margin)
            elif margin <= -THRESHOLD:
                result[fb] = _cr("persistence_wins", "recovery_time", -margin)
            else:
                result[fb] = _cr("no_advantage", "recovery_time", margin)

    return result


def compare_persistence_vs_feedback_c(metrics: dict) -> dict:
    """Exp C: Does feedback alone achieve selective reactivation (precision)?
    Compares persistence_only vs feedback_only_h8/h12 on precision, then speed.
    """
    p_prec  = metrics.get("baseline_precision", {}).get("persistence_only", 0.0)
    p_speed = metrics.get("baseline_proj_speed", {}).get("persistence_only")
    result  = {}

    for fb in ("feedback_only_h8", "feedback_only_h12"):
        fb_prec  = metrics.get("baseline_precision", {}).get(fb, 0.0)
        fb_speed = metrics.get("baseline_proj_speed", {}).get(fb)

        if fb_prec > 0 and p_prec == 0.0:
            prec_margin = 1.0
            result[fb] = _cr("feedback_wins", "precision", prec_margin)
        elif p_prec > 0 and fb_prec == 0.0:
            result[fb] = _cr("persistence_wins", "precision", 1.0)
        elif fb_prec > 0:
            prec_margin = (fb_prec - p_prec) / fb_prec
            if prec_margin >= THRESHOLD:
                result[fb] = _cr("feedback_wins", "precision", prec_margin)
            elif prec_margin <= -THRESHOLD:
                result[fb] = _cr("persistence_wins", "precision", -prec_margin)
            else:
                # Precision tied — use speed
                if p_speed is not None and fb_speed is not None and p_speed > 0:
                    speed_margin = (p_speed - fb_speed) / p_speed
                    if speed_margin >= THRESHOLD:
                        result[fb] = _cr("feedback_wins", "proj_speed", speed_margin)
                    elif speed_margin <= -THRESHOLD:
                        result[fb] = _cr("persistence_wins", "proj_speed", -speed_margin)
                    else:
                        result[fb] = _cr("no_advantage", "precision+speed", prec_margin)
                else:
                    result[fb] = _cr("no_advantage", "precision", prec_margin)
        else:
            result[fb] = _cr("no_advantage", "precision", 0.0)

    return result


def compare_persistence_vs_feedback_d(metrics: dict) -> dict:
    """Exp D: Which mechanism contributes to long-chain reactivation?
    persistence_only vs feedback_only_h8: survival (H comparison at H=f(P) vs H=8)
    persistence_only vs feedback_only_h12: reactivation speed (F contribution at same H level)
    """
    p_success = metrics.get("baseline_success_d", {}).get("persistence_only", False)
    p_speed   = metrics.get("baseline_proj_speed", {}).get("persistence_only")
    result    = {}

    for fb in ("feedback_only_h8", "feedback_only_h12"):
        fb_success = metrics.get("baseline_success_d", {}).get(fb, False)
        fb_speed   = metrics.get("baseline_proj_speed", {}).get(fb)

        if p_success and not fb_success:
            result[fb] = _cr("persistence_wins", "success_d", 1.0)
        elif not p_success and fb_success:
            result[fb] = _cr("feedback_wins", "success_d", 1.0)
        elif not p_success and not fb_success:
            result[fb] = _cr("no_advantage", "success_d", None)
        else:
            # Both succeed — compare speed
            if p_speed is not None and fb_speed is not None and p_speed > 0:
                margin = (p_speed - fb_speed) / p_speed
                if margin >= THRESHOLD:
                    result[fb] = _cr("feedback_wins", "proj_speed", margin)
                elif margin <= -THRESHOLD:
                    result[fb] = _cr("persistence_wins", "proj_speed", -margin)
                else:
                    result[fb] = _cr("no_advantage", "proj_speed", margin)
            else:
                result[fb] = _cr("no_advantage", "success_d", 0.0)
